  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <button
        ...
        disabled={spinning}
        style={{
          background: spinning ? 'var(--border)' : 'var(--purple)',
      ...
      {result && (
        <div style={{
          ...
          border: '2px solid var(--border-accent)', borderRadius: '16px',
          maxWidth: '400px', margin: '1.5rem auto 0',
        }}>
          <span style={{ fontSize: '0.75rem', color: 'var(--purple)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            {catLabels[result.cat_slug] || result.cat_slug}
          </span>
          <h3 style={{ margin: '0.5rem 0', fontSize: '1.25rem' }}>{result.name}</h3>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1rem' }}>{result.card_hook}</p>
          <a
