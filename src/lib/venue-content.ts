...
export function generateIntro(v: Venue, h: number, idx: number) {
  ...
  // Generic bullet (no STORE_NAME — template h1/h2/TrustBlock already add 3)
  const bulletWithName = `방문 전 꼭 알아야 할 실전 정보 총정리`;
...
const faqPool: Record<string, ((v: Venue) => { q: string; a: string })[]> = {
  club: [
    (v) => ({ q: `${v.station}에서 가는 가장 빠른 교통편은?`, a: `${v.station}에서 도보로 이동 가능합니다. 심야 시간대엔 택시가 빠르며, 네이버 지도에서 검색하면 정확한 경로가 나옵니다.` }),
    (v) => ({ q: `예약 없이 당일 워크인 입장이 되나요?`, a: `워크인 입장이 가능합니다. 다만 주말 자정 이후에는 대기가 생길 수 있어 가능하면 오픈 시간에 맞춰 방문하세요.` }),
    (v) => ({ q: `1인 방문이어도 분위기를 즐길 수 있을까요?`, a: `1인 방문객도 있습니다. 바 카운터에 자리를 잡으면 자연스럽게 즐길 수 있고, 바텐더와 가볍게 대화할 수도 있습니다.` }),
    (v) => ({ q: `복장이나 신발 규정은 어떤 편인가요?`, a: `스마트캐주얼 이상이면 됩니다. 운동화·슬리퍼·반바지는 입장이 제한될 수 있으니 깔끔한 신발과 단정한 복장을 추천합니다.` }),
    (v) => ({ q: `입장료와 음료 가격대는 얼마 정도인가요?`, a: `입장료는 이벤트에 따라 1~3만 원이며, 음료는 잔당 1~2만 원입니다. 입장료에 음료 1잔이 포함되는 경우도 있으니 당일 확인하세요.` }),
    (v) => ({ q: `${v.station} 근처에 주차할 곳이 있나요?`, a: `인근 유료 주차장 이용이 가능합니다. 다만 음주 예정이면 대중교통이나 택시를 강력 추천합니다.` }),
    (v) => ({ q: `금·토 분위기가 가장 뜨거운 시간대는?`, a: `금·토요일 자정~새벽 2시가 피크입니다. 평일은 비교적 여유로워 첫 방문에 적합합니다.` }),
    (v) => ({ q: `겉옷이나 짐을 맡길 클로크룸이 있나요?`, a: `대부분 클로크룸을 운영합니다. 겉옷과 큰 가방은 맡기고 폰·카드·신분증만 소지하면 편합니다.` }),
    (v) => ({ q: `밖에 나갔다 다시 들어오는 재입장이 가능한가요?`, a: `정책은 날마다 다를 수 있습니다. 입장 시 스태프에게 미리 확인하세요. 손목 밴드를 유지하면 되는 경우가 많습니다.` }),
    (v) => ({ q: `${v.station} 주변 새벽 귀가 택시 잡기가 수월한가요?`, a: `${v.station} 주변에서 택시 잡기가 가능하지만, 새벽 3시 이후에는 앱 호출이 더 확실합니다. 미리 앱을 준비해 두세요.` }),
    (v) => ({ q: `DJ가 주로 트는 음악 장르가 궁금합니다.`, a: `EDM 계열이 메인이며, 날짜와 DJ에 따라 힙합·하우스·테크노 등 다양합니다. SNS에서 그날의 라인업을 확인할 수 있습니다.` }),
    (v) => ({ q: `내부 분위기를 사진이나 영상으로 찍어도 되나요?`, a: `정책은 날마다 다릅니다. 플래시 촬영은 대부분 제한되고, 개인 기록용 촬영은 허용되는 경우가 많습니다. 입장 시 확인하세요.` }),
    (v) => ({ q: `처음 가면 바로 플로어로 나가야 하나요?`, a: `전혀 서두를 필요 없습니다. 바 근처에서 음료를 마시며 분위기를 익히고, 준비가 되면 플로어로 나가면 됩니다.` }),
    (v) => ({ q: `만 19세 미만도 입장할 수 있나요?`, a: `만 19세 이상만 입장 가능합니다. 신분증으로 나이를 확인하며, 미지참 시 입장이 거부됩니다.` }),
  ],
  night: [
    (v) => ({ q: `${v.station}에서 걸어갈 수 있는 거리인가요?`, a: `${v.address}에 위치합니다. ${v.station}에서 도보 이동 가능하며, 네이버 지도에서 검색하면 됩니다.` }),
    (v) => ({ q: `예약 없이 당일 바로 갈 수 있나요?`, a: `워크인 입장이 된다. 주말에는 일찍 도착하거나 전화 예약을 하면 원하는 자리를 확보할 수 있다.` }),
    (v) => ({ q: `1인 방문이어도 어색하지 않은 분위기인가요?`, a: `1인 방문도 자연스러운 편이다. 평일 이른 시간대가 편하고, 바 좌석이나 작은 테이블에 앉으면 된다.` }),
    (v) => ({ q: `어떤 옷차림으로 가야 입장에 문제가 없나요?`, a: `깔끔한 캐주얼이면 충분합니다. 슬리퍼·반바지 정도만 피하면 입장에 문제 없습니다.` }),
    (v) => ({ q: `1인 예산은 어느 정도 잡으면 충분할까요?`, a: `입장료 포함 1인 2~4만 원이면 충분하다. 음료 추가 주문 시 만 원 내외가 더 들 수 있다.` }),
    (v) => ({ q: `${v.station} 근처에 주차할 곳이 있나요?`, a: `인근 공영주차장 이용이 가능합니다. 음주 예정이면 대중교통을 추천합니다.` }),
    (v) => ({ q: `현장 분위기가 가장 뜨거운 시간대가 언제인가요?`, a: `주말 밤 10시~자정이 절정이다. 여유로운 분위기를 원하면 평일 저녁 8시경이 적합하다.` }),
    (v) => ({ q: `메인으로 나오는 음악 장르가 궁금합니다.`, a: `가요·트로트·댄스 뮤직이 메인이다. 시대를 넘나드는 히트곡이 플레이되어 세대 불문 즐길 수 있다.` }),
    (v) => ({ q: `테이블 지정석을 미리 잡아 둘 수 있나요?`, a: `전화로 예약하면 원하는 위치의 테이블을 배정받을 수 있습니다. 주말 방문이면 당일 오후에 연락하세요.` }),
    (v) => ({ q: `플로어에 안 나가고 앉아만 있어도 괜찮나요?`, a: `물론이다. 테이블에서 음악을 감상하며 음료를 즐기는 것도 훌륭한 방법이다. 강제 참여는 없다.` }),
    (v) => ({ q: `밴드 라이브나 DJ 공연이 있는 날도 있나요?`, a: `날짜에 따라 라이브 밴드나 DJ 공연이 있을 수 있습니다. 방문 전 전화나 SNS로 일정을 확인하세요.` }),
    (v) => ({ q: `운영 시간이 보통 새벽 몇 시까지인가요?`, a: `보통 새벽 1~3시까지 운영합니다. 정확한 마감 시간은 요일에 따라 다르니 방문 전 확인하세요.` }),
    (v) => ({ q: `처음 방문인데 현장에서 어떻게 즐기면 좋을까요?`, a: `테이블에 앉아 음료를 마시며 분위기를 살피다가, 좋아하는 노래가 나오면 플로어에 합류하면 됩니다. 자기 페이스가 가장 좋습니다.` }),
    (v) => ({ q: `만 19세 미만도 입장할 수 있나요?`, a: `만 19세 이상만 입장 가능하다. 신분증을 반드시 지참하세요.` }),
  ],
  lounge: [
    (v) => ({ q: `${v.station}에서 가는 교통편을 알려 주세요.`, a: `${v.address} 근처입니다. ${v.station}에서 도보 또는 택시로 접근 가능합니다.` }),
    (v) => ({ q: `예약은 꼭 해야 하나요, 워크인도 가능한가요?`, a: `주말 저녁은 예약을 강력 추천합니다. 주중이면 워크인도 가능하지만 자리가 보장되진 않습니다.` }),
    (v) => ({ q: `1인 방문해서 바 좌석에 앉아도 괜찮을까요?`, a: `물론입니다. 바 좌석은 1인 방문객에게 인기가 많으며, 바텐더와 대화하며 시간을 보낼 수 있습니다.` }),
    (v) => ({ q: `복장 규정(드레스코드)은 어떤 편인가요?`, a: `스마트캐주얼 이상입니다. 남성은 셔츠+슬랙스, 여성은 원피스·블라우스 정도면 무난합니다. 슬리퍼·반바지는 입장 불가입니다.` }),
    (v) => ({ q: `2인 방문 시 예산을 얼마쯤 잡으면 될까요?`, a: `음료 2잔 + 안주 포함 5~10만 원이면 넉넉합니다. 위스키 보틀 주문 시 금액이 올라갑니다.` }),
    (v) => ({ q: `${v.station} 근처에 주차나 발렛 서비스가 되나요?`, a: `일부 라운지는 발렛 서비스를 제공합니다. 없는 경우 인근 유료 주차장을 이용하세요.` }),
    (v) => ({ q: `대화하기 좋은 조용한 시간대는 언제인가요?`, a: `주중 저녁 7~9시가 가장 프라이빗합니다. 주말 심야로 갈수록 볼륨과 인원이 올라갑니다.` }),
    (v) => ({ q: `바텐더에게 메뉴 추천을 받을 수 있나요?`, a: `바텐더에게 취향(달달한/드라이/과일 향/허브 향)을 말하면 맞춤 추천을 받을 수 있습니다.` }),
    (v) => ({ q: `비즈니스 미팅 장소로도 적합한 편인가요?`, a: `주중 저녁이라면 대화에 적합한 볼륨과 분위기입니다. 사전에 조용한 테이블을 요청하면 더 좋습니다.` }),
    (v) => ({ q: `이 라운지만의 시그니처 메뉴가 따로 있나요?`, a: `대부분의 라운지에 시그니처 칵테일이 있습니다. 메뉴판에 표시되어 있으니 첫 잔으로 시도해 보세요.` }),
    (v) => ({ q: `기념일에 특별 서비스를 받을 수 있나요?`, a: `예약 시 기념일임을 알리면 케이크 세팅이나 테이블 데코레이션을 해 주는 경우가 있습니다. 사전 문의가 필요합니다.` }),
    (v) => ({ q: `음악 볼륨이 대화에 방해되지 않을 정도인가요?`, a: `라운지 특성상 대화가 가능한 수준으로 유지됩니다. 다만 주말 심야에는 볼륨이 올라갈 수 있습니다.` }),
    (v) => ({ q: `처음 방문인데 주의할 점이 있을까요?`, a: `예약, 드레스코드, 최소 주문 금액 — 이 세 가지만 확인하면 됩니다. 나머지는 가서 편하게 즐기면 됩니다.` }),
    (v) => ({ q: `보통 새벽 몇 시까지 영업하나요?`, a: `보통 새벽 1~3시까지 운영합니다. 라스트 오더는 마감 30분~1시간 전이니 미리 확인하세요.` }),
  ],
};
...
export function generateAllContent(v: Venue, idx: number): GeneratedContent {
  ...
  // Boost: add name to costNote if under 4 (template adds 3 more → total ≤ 7 minimum)
  ...
  // Extra boost: add name prefix to first scenario if still under 4
  if (nameCount < 4 && quickPlan.scenarios.length > 0) {
    quickPlan.scenarios[0].desc = `${v.name} — ${quickPlan.scenarios[0].desc}`;
    nameCount++;
  }

  // Trim: target internal count ≤ 7 (template h1 + AI summary h2 + TrustBlock add 3 → total ≤ 10)
  const TARGET_INTERNAL = 7;
  const finalAiSummary = [...aiSummary];
  const nameReTrim = new RegExp(v.name.replace(/[.*+?${}()|[\]\\]/g, '\\$&') + '\\s*', 'g');

  // Pass 1: strip from last AI summary bullet
  if (nameCount > TARGET_INTERNAL) {
    const lastIdx = finalAiSummary.length - 1;
    if (lastIdx >= 0) {
      const before = countName(finalAiSummary[lastIdx]);
      finalAiSummary[lastIdx] = finalAiSummary[lastIdx].replace(nameReTrim, '');
      nameCount -= (before - countName(finalAiSummary[lastIdx]));
    }
  }

  // Pass 2: strip from outro
  let finalOutro = outro;
  if (nameCount > TARGET_INTERNAL) {
    const before = countName(finalOutro);
    finalOutro = finalOutro.replace(nameReTrim, '');
    nameCount -= (before - countName(finalOutro));
  }

  // Pass 3: strip one mention from scene1
  let finalScene1 = scene1;
  if (nameCount > TARGET_INTERNAL && countName(finalScene1) > 0) {
    finalScene1 = finalScene1.replace(v.name, '');
    nameCount--;
  }
...
export const hookDescs: Record<string, string> = {
  ...
  'intro': '인트로라운지는 압구정 칵테일 중심의 프라이빗 라운지. 시끄럽지 않은 공간에서 소규모 모임과 조용한 2차를 즐기기 좋습니다. 영업시간·예약 방법·좌석 배치 안내.',
