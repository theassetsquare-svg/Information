...
export function generateGoldContent(venue: Venue, venueIndex = 0) {
  ...
  // description — venueIndex 기반 10단어 from 150풀 (유사도 최소화)
  const dW = [
    '답사기록','현장메모','꼼꼼정리','스크랩북','점검표','단골비법','통합요약','전문수록',
    '결정판','정밀취재','밀착르포','빽빽정보','핵심모음','단독공개','실속팁','첫발걸음',
    '엄선자료','실용활용','종합편','영업시간','절정시간','옷차림','이동수단','항목별',
    '현장감','응용법','구석구석','북마크','위치접근','경로파악','디테일','상황판단',
    '신규반영','개편내용','성실기록','최근갱신','도보탐방','생동감','순위표','신발견',
    '사전학습','목록화','비용설계','자리선택','집필내용','음료정보','주변동선','코스설계',
    '출입과정','옷규정','상세기재','합격비결','교통안내','버스노선','병렬표기','내비설정',
    '지참물품','주의사항','정렬배치','입문자용','복귀율','심층분석','기록문서','통계근거',
    '심야요령','퇴근귀가','안전보장','새벽준비','계절특성','요일분석','밀집도','시간분류',
    '필수확인','핵심요약','포인트정리','솔직후기','직접체험','현지추천','실전가이드','간략설명',
    '비교검토','재방문추천','분위기소개','시간대표기','출발점검','외출필수','즐겨찾기용','놓치면후회',
    '유용정보','간편보관','상시열람','독자후기','탐방일지','가벼운읽기','심화자료','입문해설',
    '전체조망','세부사항','참고문헌','경험공유','현장감상','솔직의견','직관적소개','핵심파악',
    '단계설명','시각자료','원포인트','분류체계','키포인트','빠른훑기','요점집약','맞춤정보',
    '실사촬영','현지답사','인터뷰','방문인증','체험보고','소감문','클리핑','편집후기',
    '리뷰모음','큐레이션','하이라이트','브리핑','미리보기','프리뷰','첫인상','마무리평',
    '전망분석','향후전망','트렌드','변화추이','신규오픈','리뉴얼','컨디션','최적타이밍',
    '예약팁','동반입장','그룹석','커플석','솔로입장','바카운터','테라스석','VIP룸',
    '사운드체크','조명연출','인테리어','공간배치','층별안내','출구위치',
  ];
  const N = dW.length;
  const primes = [1, 7, 11, 17, 23, 31, 37, 43, 53, 59];
  const finalIdx: number[] = [];
  const used = new Set<number>();
  for (let k = 0; k < 10; k++) {
    let idx = (venueIndex * primes[k] + k * 5 + 3) % N;
    while (used.has(idx)) idx = (idx + 1) % N;
    used.add(idx);
    finalIdx.push(idx);
  }
  const picked = finalIdx.map(i => dW[i]).join(' ');
