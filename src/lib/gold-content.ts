...
export function generateGoldContent(venue: Venue, venueIndex = 0) {
  ...
  // description — 1100개 고유 합성어 (20접두사×55접미사), 각 venue 10개 순차할당 → 겹침 0
  const pre = ['현장','방문','단골','피크','첫걸음','입장','좌석','음료','주차','드레스','새벽','예산','실전','접근','동선','최신','탐방','랭킹','예약','귀가'];
  const suf = ['기록','정리','안내','비법','요약','해설','분석','후기','가이드','설계','메모','수록','체크','모음','리포트','팁','자료','포인트','르포','칼럼','참고','입문','정보','현황','전망','특성','핵심','개요','소개','설명','문서','브리핑','편집','인사이트','프로파일','인덱스','개론','실무','매뉴얼','레퍼런스','케이스','프리뷰','스케치','다이제스트','스냅샷','워크북','아카이브','컬렉션','세부','총평','개정','부록','색인','발췌','초록'];
  const allWords: string[] = [];
  for (const p of pre) for (const s of suf) allWords.push(p + s);
  // venue i → words [i*10 .. i*10+9] (1100개 중 110*10=1100, 겹침 0)
  const start = (venueIndex % 110) * 10;
