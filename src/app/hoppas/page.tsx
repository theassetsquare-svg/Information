  ...
  return (
    <section className="section">
      <div className="container">
        ...
        <div className="narrow" style={{ marginBottom: '2.5rem', padding: '2rem', background: '#1A1520', borderRadius: '16px', border: '1px solid #D4AF37' }}>
          <p style={{ color: '#F7E7CE' }}>호빠는 여성 고객을 위한 프리미엄 엔터테인먼트 공간이다. 호스트가 테이블에서 대화와 서비스를 제공한다. 시스템을 미리 이해하고 가면 첫 방문도 편안하다.</p>
          <p style={{ marginTop: '1rem', color: '#F7E7CE' }}>강남, 부산, 장안동, 건대 등 주요 상권에 위치한 곳을 정리했다. 가격은 업소마다 다르니 방문 전 전화로 확인하는 것을 추천한다. {year}년 기준 {venues.length}곳.</p>
        </div>
        ...
        {regions.map(region => {
          ...
          return (
            <div key={region} style={{ marginBottom: '2.5rem' }}>
              <h2 style={{ fontSize: '1.2rem', marginBottom: '1rem', color: '#D4AF37' }}>
                {region} <span style={{ color: '#A89B80', fontSize: '0.85rem' }}>{rv.length}곳</span>
              </h2>
        ...
        <div className="narrow" style={{ marginTop: '2rem', padding: '2rem', background: '#1A1520', borderRadius: '16px', border: '1px solid #D4AF37' }}>
          <h2 style={{ color: '#D4AF37' }}>호빠 처음이세요?</h2>
          <p style={{ marginBottom: '1rem', color: '#F7E7CE' }}>친구랑 가도 좋고, 혼자 가도 괜찮다. 시스템만 알면 편하다.</p>
