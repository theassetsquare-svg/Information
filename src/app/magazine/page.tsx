...
export const metadata: Metadata = {
  ...
  description: '나이트 처음 가는 법, 클럽과 나이트의 차이, 1인 방문 후기까지. 밤문화를 즐기기 전에 읽어두면 좋은 이야기.',
  alternates: { canonical: SITE_URL + '/magazine/' },
  openGraph: {
    title: `매거진 — 밤문화 트렌드와 현장 이야기 | ${SITE_NAME}`,
    description: '나이트 처음 가는 법, 클럽과 나이트의 차이, 1인 방문 후기까지.',
...
const ARTICLES: Article[] = [
  {
    ...
    paragraphs: [
      ...
      '예산은 지역과 업소에 따라 차이가 크다. 일반적으로 테이블 세팅비, 음료, 안주 비용이 발생한다. 2~4인 기준으로 최소 20만 원에서 50만 원 사이를 예상하면 된다. 처음이라면 전화로 미리 가격대를 확인하는 편이 현명하다.',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      '가격 구조도 다르다. 클럽은 입장료를 내고 안에서 음료를 별도로 구매하는 방식이 일반적이다. 입장료는 1만 원에서 3만 원 선이고, 음료는 1잔에 만 원 내외다. 나이트는 테이블 세팅비가 기본이므로 전체 비용이 더 높은 편이다.',
